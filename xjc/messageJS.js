...
//* Included jquery *//
var xchuangmessagenum;
var xchuangmessagestyleCode;

function startXmessage(config = { top: 0, pos: 'right', during: '.6s', background: 'linear-gradient(90deg, #66abff 2.08%, #FFFFFF 2.09%)', fontSize: 20 }) {
    if (config.pos == 'right') ops = 'left';
    else ops = 'right';
    xchuangmessagenum = 0;
    xchuangmessagestyleCode = `
    #xchuangcmessagetag {
        position: fixed;
        top: ${config.top};
        ${config.pos}: 0;
        transition: ${config.during};
        z-index: 99999;
    }
    .message {
        overflow: hidden;
        ...
        padding-right: 40px;
        height: 50px;
        border-top-${ops}-radius: 5px;
        border-bottom-${ops}-radius: 5px;
        transition: all ${config.during};
        color: black;
        ...
        border-${config.pos}: none;
    }`;
    var xchuangcodestyle = document.createElement('style');
    xchuangcodestyle.type = 'text/css';
    xchuangcodestyle.innerHTML = xchuangmessagestyleCode;
    document.getElementsByTagName('head')[0].appendChild(xchuangcodestyle);
    var xchuangcmessagetag = document.createElement("div");
    xchuangcmessagetag.setAttribute("id", "xchuangcmessagetag");
    document.body.appendChild(xchuangcmessagetag);
}

function message(m = '', type = 'tab', time = 2000, width = 200, padding = 5, paddingLeft = 40, paddingRight = 40) {
    ...
    if (type == "tab") {
        var id = "message_" + xchuangmessagenum;
        var codea = document.createElement("div");
        codea.setAttribute("id", id);
        codea.setAttribute("class", "ownmessageTag");
        document.getElementById("xchuangcmessagetag").appendChild(codea);
        var tag = document.createElement("div");
        tag.className = "message";
        tag.innerHTML = m;
        document.getElementById(id).appendChild(tag);
        $("#" + id).append('<br class="brxchuangcmessage">');
        xchuangmessagenum++;
        tag.style.width = "0";
        ...
        setTimeout(function() {
            ...
            tag.style.padding = `${padding}px`;
            tag.style.paddingLeft = `${paddingLeft}px`;
            tag.style.paddingRight = `${paddingRight}px`;
            setTimeout(function() {
                tag.style.width = "1px";
                tag.style.padding = '0px';
                tag.style.paddingLeft = '0px';
...
// (c) xchuang 2022. All Rights Reserved.
// Most of the codes are created by xchuang corp.